...
Figure~\ref{fig:flowchart} has a diagram that shows the workflow steps followed in the methodology of this project. It also explains the structure of the thesis: each step corresponds to a self-contained chapter, including a theoretical overview and practical solutions implemented in this project. The order of chapters is motivated by the perceived importance of the respective contributions rather than their chronological sequence shown in Figure~\ref{fig:flowchart}. 
...
%		module minimum height=1cm,% initial: 1cm
%		module minimum width=2cm,% initial: 2cm
%		module x sep=3} % initial 2.75
%	
%	\smartdiagram[flow diagram:horizontal]{teaching, RusLTC,
%		errors/grades, CoLing research}
%\end{center}
% from https://tex.stackexchange.com/questions/574742/can-i-include-a-vertical-smartdiagram-addition-below-horizontal-smartdiagram
\begin{figure}
	\centering
	\begin{tikzpicture}[n/.style={minimum height=9mm, minimum width=3cm,draw=cyan,thick,fill=cyan!20,rounded corners,inner xsep=1mm}]
		\def\a{7.5} % vertical distance between nodes
		\def\b{1.5} % horisontal distance between nodes
		\path[nodes={n}]
		(0,0) node (s3) [draw=red]{Feature engineering and extraction (Chapter~\ref{cha:indicators})}
		++(-\a,0) node (A) [fill=white]{Translationese studies}
		(s3)		
		++(0,-\b) node (s4) {Translation detection tasks (Chapter~\ref{cha:translationese})}
		(s4)
		++(-\a,0) node (D) [fill=white]{\gls{ML}, feature selection}
		(s4)
		++(0,-\b) node (s5) {Translation quality estimation tasks (Chapter~\ref{cha:pro_qua})}
		(s3)		
		++(0,\b) node (s2) {Obtaining quality labels/scores (Chapter~\ref{cha:quest})}
		(s2)
		++(-\a,0) node (B) [fill=white]{Quality research, Best practices}
		(s2)
		++(0,\b) node (s1) {Data collection (Chapter~\ref{cha:varieties})} 
		(s1)
		++(-\a,0) node (C) [fill=white]{Corpus and variational linguistics}
		(s1)
		;
		\foreach \p/\q in {A/s3,s2/s3,s1/s2,C/s1,B/s2,D/s4,D/s5,s3/s4,s4/s5}
		\draw[-stealth] (\p)--(\q);
	\end{tikzpicture} 
	\caption{\label{fig:flowchart}Major steps in this thesis workflow}
\end{figure}

%Both examiners agreed that the thesis would benefit from the addition of a concise explanation, e.g. by means of a framework/diagram/graphic and/or a few paragraphs, of the various workflow steps followed in the methodology. This additional text/graphic should focus only on the important milestones of the methodology followed, e.g. collection of student corpora, professional translators corpora, creation of spreadsheet with labels, meta descriptors and numeric features, how this spreadsheet was used, with which tools/models, etc, and these can be cross-referenced to the section(s) in the thesis where they are explained further/in more detail. This additional explanation of the methodology can be added to the Experimental Setup section (section 5.1), for example.
...
\label{pg:embeddings}
\begin{description}\compresslist{}
	\item[stsb-xlm-r-m:] a multilingual model\wlvfootnote{\url{https://huggingface.co/sentence-transformers/stsb-xlm-r-multilingual}} trained using Sentence-BERT (SBERT), a~\gls{SOTA} framework to compute sentence representation~\cite{Reimers2019}. SBERT is a generic name for contextualised embedding models fine-tuned on various sentence-level tasks such as question-answering, sentiment, paraphrasing, etc. The model is a multilingual XLM-R fine-tuned on \gls{STS} task,
	
	\item[TQmono-m:] a cross-lingual model for many languages\wlvfootnote{\url{https://TransQuest/monotransquest-da-multilingual}} trained by the more successful architecture from \textit{TransQuest} package~\cite{Ranasinghe2020}, to solve MT quality estimation task based on DA scores,
	% these models generate word embeddings that are mean-pooled under the hood by SentenceTransformers library 
	
	\item[ruRoberta:] a dedicated Russian model\wlvfootnote{\url{https://huggingface.co/sberbank-ai/ruRoberta-large}} trained as a full-mask language model using \textit{RoBERTa}, an optimised BERT transformers training method proposed by~\citet{Liu2019},
	
	\item[mdeberta3:] a multilingual version of \textit{DeBERTa}~\cite{He2021}, an improvement on \textit{BERT} and~\textit{RoBERTa} word embedding architectures, trained on 2.5T CommonCrawl corpus for 100 languages\wlvfootnote{\url{https://huggingface.co/microsoft/mdeberta-v3-base}}. At the time of writing, it is \gls{SOTA} in word-level representation learning. 
\end{description}
...
For quality control, we implemented a simple sequential neural model to replicate the performance of SVM-based classifiers on exactly the same inputs. The neural model~\label{pg:neural} was designed using \textit{Keras} \gls{API} integrated into \textit{TensorFlow 2.0} (tf.keras)\wlvfootnote{Both \textit{TensorFlow} (\url{https://github.com/tensorflow/tensorflow}) and \textit{Keras} (\url{https://keras.io/}) are open-source libraries for deep learning with Python.}. The neural model used document vectors as input and was compiled with two dense (i.e. fully-connected) hidden layers. The first layer was  made of 16 neurons and was activated with \gls{ReLU} function, which output zero for negative input values and returned the input directly if the input was positive. The output layer for the classification task was activated with a \textit{sigmoid} function, which mapped all inputs in the real number domain into the range of 0 to 1. % It transformed input values that were much larger than 1.0 to 1.0, values much smaller than 0.0 were snapped to 0.0. 
